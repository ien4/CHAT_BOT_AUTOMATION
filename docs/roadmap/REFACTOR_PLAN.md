...
Đây là docs/status reorganization prompt, không phải source refactor.

Kết quả:

- Docs/report đã được move vật lý sang cấu trúc theo nhóm, không còn gom mọi file ở root.
- Root `docs/` và root `report/` chỉ giữ `README.md`.
- `docs/status/PROJECT_PROGRESS_MASTER.md` trở thành file cần đọc đầu tiên cho prompt sau.
- `docs/index/CURRENT_STATUS_INDEX.md`, `docs/status/PROJECT_STATUS_MASTER.md`, `docs/index/DOCS_REPORT_ORGANIZATION_MAP.md` đã phản ánh trạng thái sau move.
- Broken-link check trong `docs/` và `report/` PASS.
- Dashboard/backend validation và full runtime gate PASS.

Không làm:

- Không sửa backend/dashboard source, schema/migration/package/lock/env.
- Không tạo per-file stub vì current references đã được cập nhật sang path mới.
- Không claim Phase 21 Done toàn bộ; backend route consolidation còn nợ.
- Không claim Meta verified/production ready.

Next:

1. `21B-5-SAFE` hoặc `NO_SAFE_CANDIDATE` cho backend route consolidation.
2. Dashboard split kế tiếp chỉ khi có prompt riêng và smoke gate đầy đủ.
3. Meta verify operator checkpoint nếu có xác nhận từ người vận hành.
...
- Tạo `docs/status/PROJECT_GOALS_AND_FACEBOOK_WEBHOOK_STATUS.md` để theo dõi goal map, readiness theo tầng, checklist trước Meta thật và checklist trước production rollout.
...
- Tạo `docs/roadmap/PROJECT_STRUCTURE_CONSOLIDATION_PLAN.md` với structure map + active risks + đề xuất Phase 21B/21C/21D.
...
- Tạo `docs/policies/QUALITY_GATE.md` (required checks, lint status, smoke link).
- Production smoke dry-run local PASS 9/9 (temp admin `test-10c-*`, cleanup=0).
- Phase 19 readiness: chọn candidate #1 = `dashboard/src/app/dashboard/analytics/page.tsx` (read-only, backend harden 09B, đã có `features/analytics/`); kế hoạch `docs/roadmap/PHASE_19_DASHBOARD_FEATURE_SPLIT_PLAN.md`. Chưa sửa page.

Warning:

- ESLint chưa cài ở cả backend/dashboard → lint non-interactive BLOCKED (OPTION C). Không tự `npm install`/sửa lock. Hướng dẫn bật lint ở `docs/policies/QUALITY_GATE.md` cho prompt dependency riêng.
...
- **Deploy docs mới**: `docs/policies/DEPLOYMENT_POLICY.md`, `docs/runbooks/PRODUCTION_ROLLOUT_CHECKLIST.md`.
...
- Sync `docs/status/PROJECT_PROGRESS.md`: Phase 18 RAG/raw SQL → Done; trạng thái hiện tại → Prompt 10A; rủi ro raw SQL unsafe → Closed.
...
- Cập nhật `docs/policies/ENV_POLICY.md` để Chatwoot env là legacy/deprecated, không thuộc target mới.
- Tạo `docs/roadmap/NO_CHATWOOT_SCHEMA_ENV_CLEANUP_PLAN.md` với bảng field legacy, migration strategy, dashboard/backend/devops blockers.
...
| Env/config | `backend/.env.example`, `dashboard/.env.example`, `dashboard/src/lib/config/env.ts`, `docs/policies/ENV_POLICY.md`, config warning helper | Prompt 08C/08D: bỏ `CHATWOOT_*`, `NEXT_PUBLIC_CHATWOOT_URL`, giữ policy không secret. |
...
| Docs current | `docs/architecture/ARCHITECTURE.md`, `docs/policies/ENV_POLICY.md`, `docs/architecture/FEATURE_INVENTORY.md`, `MULTITENANT_PROGRESS.md` | Cập nhật dần, không rewrite historical evidence bừa. |
...
- Tạo `docs/architecture/FEATURE_INVENTORY.md` và `docs/runbooks/LOCAL_RUN_GUIDE.md`.
...
- Tạo `docs/policies/ENV_POLICY.md`.
...
- `docs/architecture/ARCHITECTURE.md`, `docs/status/PROJECT_PROGRESS.md`, `docs/status/FEATURE_AUDIT_CHECKLIST.md`
...
- `report/phase-17/PROMPT_08H_BROWSER_LOGIN_REDIRECT_FIX_REPORT.md`
