...
Prompt gần nhất: **21Y — Docs/report physical reorganization + project progress master + global bug regression gate**
...
| Backend route consolidation | STARTED | Prompts/settings/quick-reply-menus/channel-configs/campaigns/stats read routes đã tách. |
...
| Nhóm | Tình trạng |
|---|---|
| Backend route consolidation Phase 21 | Còn nợ trong `backend/src/api/dashboard.js`; chỉ tiếp tục nếu có route GET/read-only nhỏ, không external/mutation/raw SQL/secret. |
...
1. `21B-5-SAFE` nếu audit chứng minh có route backend GET/read-only nhỏ còn lại.
