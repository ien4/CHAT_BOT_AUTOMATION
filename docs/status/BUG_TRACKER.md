...
| BUG-BE-02-CI | Reproduce CI local (gh unavailable) | GitHub Actions backend job đỏ: `prisma validate` fail `P1012 Environment variable not found: DATABASE_URL` (CI không có `.env`) | Cả 2 job CI có thể fail → không gate được PR/push | `PRISMA_VALIDATE_MISSING_DATABASE_URL` (root cause D) | FIXED (chờ Actions xác nhận xanh) | Thêm placeholder `DATABASE_URL` cấp job cho backend trong `ci.yml` (không secret, validate không kết nối DB) | env-less + dummy `DATABASE_URL` → `schema is valid` exit 0; backend quality + dashboard build PASS | `report/phase-21/PROMPT_BE_02_CI_AND_LOG_REDACTION_REPORT.md` |
