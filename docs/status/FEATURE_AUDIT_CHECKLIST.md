...
| Hạng mục | Trạng thái | Bằng chứng | Ghi chú |
|---|---|---|---|
| Scope guard | PASS | diff source | Chỉ docs/report, không sửa backend/dashboard source/schema/package/env. |
| Physical docs move | PASS | `git mv` | Root `docs/` chỉ giữ `README.md`; docs thật nằm trong nhóm `status/index/roadmap/runbooks/policies/architecture/archive`. |
| Physical reports move | PASS | `git mv` | Root `report/` chỉ giữ `README.md`; reports nằm theo phase/bugs/archive. |
| Progress master | PASS | `docs/status/PROJECT_PROGRESS_MASTER.md` | Có đã xử lý, đang xử lý, roadmap, blocker, bug, next action. |
| Index/status refresh | PASS | current index/status/map | 21Y là trạng thái mới nhất; không còn note "chưa move" trong source-of-truth. |
| Broken link check | PASS | scan `docs/` + `report/` | Các path `docs/<path>.md` và `report/<path>.md` trong phạm vi docs/report tồn tại sau move. |
| Dashboard validation | PASS | typecheck/build | Không phát sinh lỗi build từ docs move. |
| Backend validation | PASS | quality + Prisma validate | Backend không sửa nhưng vẫn validate baseline. |
| Runtime regression gate | PASS | fresh dev server + route/static smoke | Full dashboard routes không 500; fake route 404 hợp lệ; static assets 200. |
| Dev log scan | PASS | fresh server log | Không có missing chunk, `MODULE_NOT_FOUND`, `reading 'call'`, `_next/static` 404 hoặc route 500. |

Kết luận: Prompt 21Y hoàn tất tổ chức docs/report và không phát hiện bug runtime mới.
...
| Hạng mục | Trạng thái | Bằng chứng | Ghi chú |
|---|---|---|---|
| Candidate selection | Done | `quick-replies/page.tsx` 182 LOC | Page nhỏ/rủi ro thấp hơn settings/knowledge/handoff/tenants. |
| Page audit | Done | list/read + create/update/delete + modal form | Không external; mutation phải khóa trong smoke. |
| Feature folder | Created | `dashboard/src/features/quick-replies/**` | Hook + components + types + formatter + barrel; README cập nhật. |
| Page orchestrator | Done | `page.tsx` còn 52 LOC | Giữ `'use client'`, route cũ và layout cũ. |
| API contract | Preserved | dùng `quickReplyMenusApi` cũ | Không sửa `dashboard/src/lib/api.ts`; URL/payload/error text giữ nguyên. |
| Mutation handlers | Preserved | `create/update/delete` trong hook | **LOCKED_NOT_EXECUTED** trong runtime smoke. |
| Dashboard validation | PASS | `tsc --noEmit`, typecheck, build | Build route `/dashboard/quick-replies` PASS. |
| Backend validation | PASS | `npm run quality`, `npx prisma validate` | Backend không sửa nhưng vẫn giữ baseline. |
| Clean `.next` proof | PASS | remove ignored `.next`, rebuild | Bắt buộc vì lịch sử stale cache 21C-FIX. |
| Runtime smoke | PASS | dev server tạm port `3019` | Login/dashboard/quick-replies/content-packages/prompts/analytics/settings/campaigns 200; route giả 404. |
| Chunk regression | PASS | dev log sau smoke | Không có missing chunk, `reading 'call'`, vendor ENOENT hoặc static chunk 404 lỗi. |
| Safety scans | PASS WITH HISTORICAL MATCHES | `rg` theo prompt | Không direct fetch/env/secret trong feature; install/destructive/chunk matches là docs/script/report lịch sử cũ. |
| Forbidden areas | Unchanged | diff guard | Không sửa backend/schema/package/webhook/RAG/handoff/tenants/Docker/env thật. |

Kết luận: dashboard `quick-replies` đã tách an toàn theo pattern Phase 19. Behavior/API/UI giữ nguyên; mutation chỉ preserve, không executed.
...
| Staging runbook | Created | `docs/runbooks/META_WEBHOOK_STAGING_RUNBOOK.md` | Public smoke/verify/rollback hướng dẫn rõ. |
...
| Hạng mục | Trạng thái | Bằng chứng | Ghi chú |
|---|---|---|---|
| Staging readiness doc | Created | `docs/status/META_WEBHOOK_STAGING_READINESS.md` | Ghi đúng callback `https://<domain>/webhook` và checklist staging. |
...
| Hạng mục | Trạng thái | Bằng chứng | Ghi chú |
|---|---|---|---|
| Current docs index | Created | `docs/index/CURRENT_STATUS_INDEX.md` | Nguồn vào hiện tại cho status/architecture/webhook/deploy/quality. |
| Historical docs index | Created | `docs/index/HISTORICAL_DOCS_INDEX.md` | Phân loại reports/root stale docs và cách đọc tài liệu cũ. |
...
| Hạng mục | Trạng thái | Bằng chứng | Ghi chú |
|---|---|---|---|
| Goal status captured | Done | `docs/status/PROJECT_GOALS_AND_FACEBOOK_WEBHOOK_STATUS.md` | Ghi rõ product/architecture/security/runtime/structure/devops/webhook goals. |
| No-Chatwoot target | Confirmed | Phase 17 docs + `docs/policies/DEPLOYMENT_POLICY.md` | Target là direct Facebook -> backend Express, không dùng Chatwoot. |
...
| Hạng mục | Trạng thái | Bằng chứng | Ghi chú |
|---|---|---|---|
| Structure audit | Done | `docs/roadmap/PROJECT_STRUCTURE_CONSOLIDATION_PLAN.md` | Map backend/dashboard/docs/report/scripts. |
...
| Dashboard lint non-interactive | BLOCKED (dependency) | — | ESLint chưa cài → OPTION C; hướng dẫn cài ở `docs/policies/QUALITY_GATE.md`; không tự install. |
...
| Phase 19 readiness | Done | `docs/roadmap/PHASE_19_DASHBOARD_FEATURE_SPLIT_PLAN.md` | Candidate #1 = `analytics/page.tsx` (read-only, 374 LOC, backend harden 09B). Chưa sửa page. |
| Docs quality gate | Created | `docs/policies/QUALITY_GATE.md` | Required checks + lint status + smoke link. |
...
| Deploy docs | Created | `docs/policies/DEPLOYMENT_POLICY.md`, `docs/runbooks/PRODUCTION_ROLLOUT_CHECKLIST.md` | Backup/migrate/rollback/health/webhook checklist. |
...
| Env policy | Updated | `docs/policies/ENV_POLICY.md` | Ghi rõ Chatwoot env là legacy/deprecated và không thuộc target mới. |
| Schema cleanup plan | Created | `docs/roadmap/NO_CHATWOOT_SCHEMA_ENV_CLEANUP_PLAN.md` | Có bảng field legacy, migration strategy, dashboard/backend/devops blockers. |
...
| Hạng mục | Trạng thái | Bằng chứng | Ghi chú |
|---|---|---|---|
| No-Chatwoot directive | Captured | `docs/architecture/NO_CHATWOOT_DIRECT_ARCHITECTURE_CONTEXT.md` | Chatwoot bị loại khỏi kiến trúc đích; không sinh thêm Chatwoot route/controller/service/model/env mới. |
...
| Env/config blockers | OPEN | `backend/.env.example`, `dashboard/.env.example`, `dashboard/src/lib/config/env.ts`, `docs/policies/ENV_POLICY.md`, `backend/src/infrastructure/services/config.js` | Còn `CHATWOOT_*` và `NEXT_PUBLIC_CHATWOOT_URL`; target mới không được thêm biến Chatwoot. |
...
| Docs/current architecture | OPEN | `docs/architecture/ARCHITECTURE.md`, `docs/policies/ENV_POLICY.md`, `docs/architecture/FEATURE_INVENTORY.md`, `MULTITENANT_PROGRESS.md` còn mô tả Chatwoot | Tài liệu mới phải coi Chatwoot deprecated/removed target; historical docs/report không rewrite hàng loạt. |
...
| Hạng mục | Trạng thái | Bằng chứng | Ghi chú |
|---|---|---|---|
| Feature inventory | Done | `docs/architecture/FEATURE_INVENTORY.md` | 9 nhóm chức năng; không mục nào đánh dấu runtime PASS. |
| Local run guide | Done | `docs/runbooks/LOCAL_RUN_GUIDE.md` | Trạng thái readiness + checklist thủ công + lệnh an toàn. |
...
|---|---|---|---|
| Prompt 01 audit | Done | `report/archive/early-prompts/PROMPT_01_PROJECT_AUDIT_CLEAN_ARCHITECTURE_REPORT.md` | Read-only audit, không sửa source runtime. |
| Prompt 02 safety gate | Done / Blocked đúng guardrail | `report/archive/early-prompts/PROMPT_02_BLOCKED_NEED_GIT_CHECKPOINT.md` | Việc dừng vì thiếu `.git` là đúng quy trình. |
| Prompt 02A progress/checklist foundation | Done | `docs/status/PROJECT_PROGRESS.md`, `docs/status/FEATURE_AUDIT_CHECKLIST.md`, report Prompt 02A | Chỉ docs/report, không runtime source. |
...
| Prompt 04A progress rewrite | Done | `docs/status/PROJECT_PROGRESS.md`, report Prompt 04A | Chỉ docs/report; dùng để vào Prompt 05 rõ hơn. |
| Runtime verification | Not done | Chưa chạy app/API thật | Không đánh dấu feature runtime là DONE cho đến khi có smoke test. |
| Prompt 05 readiness | Ready with warnings | `docs/roadmap/REFACTOR_PLAN.md`, `docs/policies/ENV_POLICY.md`, report Prompt 04 | Chỉ tách route/controller domain nhỏ trong `backend/src/api/dashboard.js`. |
...
| Env policy | Đã tạo quy tắc local vs production, public vs secret, validation an toàn | Documentation pass | `docs/policies/ENV_POLICY.md` | Dùng làm guardrail cho Prompt 05+. |
...
| Appointment tools | Đã có | `backend/src/bot/tools.js`, `backend/src/api/dashboard.js`, `docs/archive/unclassified/appointment-modify-spec.md` | Docs có thể lệch code; update/reschedule/cancel cần test | So sánh spec với route/tool thực tế | P0 | Cần không làm mất flow đặt lịch. |
...
- [ ] Cập nhật trạng thái trong `docs/status/PROJECT_PROGRESS.md`.
