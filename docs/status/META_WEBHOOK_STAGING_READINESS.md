...
Runbook vận hành staging: `docs/runbooks/META_WEBHOOK_STAGING_RUNBOOK.md`.
...
| Production rollout | PRODUCTION_PENDING | Chưa backup + `prisma migrate deploy` + smoke production thật | Dùng `docs/runbooks/PRODUCTION_ROLLOUT_CHECKLIST.md` khi rollout. |
...
- Staging runbook đã được tạo tại `docs/runbooks/META_WEBHOOK_STAGING_RUNBOOK.md`; cần dùng runbook này khi rollback proxy/env/app.
