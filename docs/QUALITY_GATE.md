...
Quality gate cho local/CI trước khi merge/deploy. Không thay thế `docs/DEPLOYMENT_POLICY.md` (deploy) — bổ sung cho nó ở tầng validation code.
...
- Sau đó theo `docs/PRODUCTION_ROLLOUT_CHECKLIST.md` (backup → `prisma migrate deploy` release step → deploy → health/smoke).
