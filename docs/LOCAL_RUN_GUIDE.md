...
**Kết luận readiness:** Ở trạng thái mặc định (không có `.env`) chưa đủ điều kiện. Prompt 05R-ENV đã chứng minh có thể tạo môi trường test dùng một lần và đã **runtime smoke test PASS** cho 3 route (xem mục 8).

## 1b. Đã verify runtime (Prompt 05R-ENV, DB test dùng một lần)

3 route đã tách được runtime verified PASS bằng cách:
- Tạo `backend/.env` + `dashboard/.env.local` local-only (gitignored, không commit).
- Chạy 1 container Postgres tạm: `docker run -d --name bbotest_pg -e POSTGRES_USER=bbotest -e POSTGRES_PASSWORD=<local> -e POSTGRES_DB=bbotest -p 5433:5432 pgvector/pgvector:pg16`.
- `DATABASE_URL=postgresql://bbotest:<local>@localhost:5433/bbotest`.
- `cd backend && npx prisma migrate deploy` (áp migration sẵn có, non-destructive — **không** `db push`).
- `node src/index.js`, login `POST /api/auth/login` để lấy token, gọi 3 route.
- Sau test: gỡ container tạm + xóa env local.
...
Kết quả runtime đã xác minh (Prompt 05R-ENV):
- Không có token → **401**, không crash. ✅
- Có token hợp lệ:
  - `/api/settings/webhook` → **200**, JSON `{ verifyToken, pageAccessToken, appSecret, webhookUrl }` (secret mask `***configured***` hoặc `null`). ✅
