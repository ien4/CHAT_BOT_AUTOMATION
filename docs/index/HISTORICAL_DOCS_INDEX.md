...
Trước khi đọc tài liệu cũ, đọc `docs/index/CURRENT_STATUS_INDEX.md` và `docs/status/PROJECT_GOALS_AND_FACEBOOK_WEBHOOK_STATUS.md` để biết source of truth hiện tại.

## 2. Historical reports

| Nhóm | File | Trạng thái | Cách đọc |
|---|---|---|---|
| Prompt reports | `report/README.md` | HISTORICAL_REPORT | Root report là index; report thật nằm theo phase/bugs/archive sau Prompt 21Y. |
| Early prompt reports | `report/archive/early-prompts/` | HISTORICAL_REPORT | Prompts 01-06 và 05R, giữ làm bằng chứng nền. |
| No-Chatwoot reports | `report/phase-17/` | HISTORICAL_REPORT | Dùng để trace quá trình bỏ Chatwoot; trạng thái cuối cùng phải đối chiếu với current status docs. |
| Runtime/readiness reports | `report/phase-21/`, `report/phase-22/`, `report/bugs/` | RECENT_HISTORY | Gần hiện tại hơn, nhưng vẫn là report theo thời điểm. |
...
| File | Caveat |
|---|---|
| `docs/architecture/ARCHITECTURE.md` | Có phần architecture shell lịch sử còn nhắc Chatwoot như integration cũ; đọc kèm `docs/index/CURRENT_STATUS_INDEX.md`. |
| `docs/architecture/FEATURE_INVENTORY.md` | Có inventory từ giai đoạn đầu còn mô tả Chatwoot và `db push` risk; không dùng làm trạng thái hiện tại nếu mâu thuẫn docs mới. |
...
- Ưu tiên `docs/index/CURRENT_STATUS_INDEX.md` trước.
- Ưu tiên `docs/status/PROJECT_GOALS_AND_FACEBOOK_WEBHOOK_STATUS.md` cho webhook/Meta/production status.
...
- Không xóa historical reports; sau Prompt 21Y chỉ move vật lý và update index/link.
