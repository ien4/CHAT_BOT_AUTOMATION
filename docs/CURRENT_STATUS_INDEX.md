...
- Dashboard feature split Phase 19 đang **Started**, chưa Done. Analytics/prompts/staff/appointments/content-packages/quick-replies đã split; các page nặng còn lại cần prompt riêng.
...
| 21D/21B-5 hoặc dashboard split kế tiếp | Sau 21C-2, chỉ tiếp tục backend route read-only nhỏ nếu audit đủ an toàn, hoặc dashboard page nhỏ như `campaigns` với upload/write bị khóa rõ. |
