...
| Status hub hiện tại | `docs/PROJECT_STATUS_MASTER.md` | Bảng trạng thái master theo nhóm, phase board, next prompts và các điều không được claim. |
| Bug tracker | `docs/BUG_TRACKER.md` | Tracker bug runtime hiện tại và quy tắc xử lý bug dashboard. |
| Dashboard route health | `docs/DASHBOARD_ROUTE_SMOKE_MATRIX.md` | Matrix smoke route/static assets/dev log mới nhất. |
| Docs/report organization | `docs/DOCS_REPORT_ORGANIZATION_MAP.md` | Bản đồ tổ chức docs/report và kế hoạch move không gãy link. |
...
- Prompt 21C-FIX đã PASS: bug runtime chunk sau 21C được xác định là **STALE_NEXT_DEV_CACHE_RESOLVED**; sau khi dừng dev server cũ, xóa `.next`, rebuild và fresh dev smoke port `3019`, các route `/dashboard`, `/dashboard/content-packages`, `/dashboard/settings`, `/dashboard/campaigns`, `/dashboard/conversations`, `/dashboard/knowledge`, `/dashboard/prompts`, `/dashboard/analytics` đều không còn 500/chunk error.
- Prompt 21C-2-SAFE đã PASS: `dashboard/src/app/dashboard/quick-replies/page.tsx` giảm từ 182 LOC xuống 52 LOC, feature nằm ở `dashboard/src/features/quick-replies/**`; mutation create/update/delete được preserve nhưng **LOCKED_NOT_EXECUTED** trong smoke; clean `.next` + fresh route smoke không tái hiện chunk error.
- Prompt 21C-3-SAFE đã PASS: `dashboard/src/app/dashboard/campaigns/page.tsx` giảm từ 196 LOC xuống 57 LOC, feature nằm ở `dashboard/src/features/campaigns/**`; upload/create/update/delete được preserve nhưng **LOCKED_NOT_EXECUTED** trong smoke; clean `.next` + fresh route smoke không tái hiện chunk error.
- Prompt 21X đã PASS: `Bug_21C-3.md` được phân loại **MIXED_DEV_SERVER_OR_PORT + STALE_NEXT_DEV_CACHE**; dừng đúng dashboard dev server cũ `3002` PID `20916/3524`, clean `.next`, rebuild, fresh dev server `3019`, full dashboard route smoke PASS, `/dashboard/tenants` 200, 125 static assets đều 200, dev log không còn missing chunk/static 404/cache ENOENT.
- Docs/status hub đã tạo: `docs/PROJECT_STATUS_MASTER.md`, `docs/BUG_TRACKER.md`, `docs/DASHBOARD_ROUTE_SMOKE_MATRIX.md`, `docs/DOCS_REPORT_ORGANIZATION_MAP.md`, `report/README.md` và các README phase/bugs. Chưa move historical docs/report để tránh gãy link.
...
- Sau mọi dashboard refactor: bắt buộc clean `.next`, fresh dev server, full route smoke, static asset smoke và dev log scan. Không code bậy, không sửa chắp vá.
...
| 21Y-DOCS-ARCHIVE-MOVE | Move/copy docs/report vào cấu trúc đề xuất với compatibility stubs hoặc link updates, không làm gãy audit trail. |
| 21B-5 hoặc NO_SAFE_CANDIDATE | Chỉ tiếp tục backend read-only route consolidation nếu audit tìm được candidate nhỏ, không external/mutation/raw SQL/secret; nếu không thì dừng rõ. |
| Dashboard split kế tiếp | Chỉ sau khi đọc `docs/BUG_TRACKER.md` và áp dụng rule 21X: full route smoke + static asset smoke + dev log scan. |
