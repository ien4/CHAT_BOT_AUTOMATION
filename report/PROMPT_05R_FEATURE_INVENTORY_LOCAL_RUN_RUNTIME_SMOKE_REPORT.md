...
- `docs/ENV_POLICY.md`
- `report/PROMPT_05C_BACKEND_API_ROUTE_CONTROLLER_SPLIT_REPORT.md`
...
- **Manual steps required**: tạo `backend/.env` + `dashboard/.env(.local)` từ example; cung cấp DB local/test + `DATABASE_URL`; `JWT_SECRET`; `ENCRYPTION_KEY` nếu cần; credential admin local/test; không dùng token production. Chi tiết trong `docs/LOCAL_RUN_GUIDE.md` mục 3.
...
Người dùng cần chuẩn bị (chi tiết `docs/LOCAL_RUN_GUIDE.md` mục 3):
...
- `report/PROMPT_05R_FEATURE_INVENTORY_LOCAL_RUN_RUNTIME_SMOKE_REPORT.md`
