...
- Hướng dẫn bật lint (ESLint 8 + `eslint-config-next@14`, `.eslintrc.json`, `next lint --max-warnings=0`) ghi ở `docs/QUALITY_GATE.md` cho prompt dependency riêng.
...
```
Chi tiết + required checks: `docs/QUALITY_GATE.md`.
...
**Candidate #1 = `dashboard/src/app/dashboard/analytics/page.tsx`**: read-only (`analyticsApi.get`), backend harden 09B, có sẵn `features/analytics/`, build route PASS. Kế hoạch chi tiết: `docs/PHASE_19_DASHBOARD_FEATURE_SPLIT_PLAN.md`. **Chưa sửa page trong 10C.**
